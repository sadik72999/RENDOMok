...
def line():
	print(40*"=")
############------[ RANDOM SYS ]------#########
BDX=f"{W}MASTER CARD CODE : {G}017 015 018 019 013 016{E}{W}"
INDX=f"{W}VISA CARD CODE : {G}9670 9725 8948 8795 6383{E}{W}"
PAKX=f"{W}AMEX CARD CODE : {G}0306 0315 0335 0345 0318{E}{W}"
LIMITX=f"EXAMPLE : {G}1000{W},{G}5000{W},{G}10000{W},{G}15000{W},{G}20000{W}"
############------[ A SYS ]------#########
CPG=f"[{G}+{W}] Do you have a Binance account (y/n)"
CKIG=f"[{G}+{W}] Do you want information with the card (y/n)"
...
############------[ LOGO ]------#########
os.system('espeak -a 300 "well,come to, dark,web, site"')
def logo():
...
{40*"="} \x1b[1;97m
 [{G}+{W}] Site Name: Dark Web Card Hack
 [{G}+{W}] Contact: darkwebcc9@gmail.com    
 [{G}+{W}] Card Balance: $100–$700 per card
 [{G}+{W}] Tool Key: aksjdinci
 
 
 [{G}+{W}] Prices: 30 DAYS = 120$ 60 DAYS = 150$ 120 DAYS = 220$
 
 
 [{G}+{W}] payment (Optimism): 0xf718384fba0e1410e1b6589a9cd853deaa8f02f3 
 [{G}+{W}] payment (Aptos): 0xbb660103d1a23dc0de2c033c3ec0d5e1f5cdd4cf5aaf5997780fc69c8f861c0a
 [{G}+{W}] payment (CELO): 0xf718384fba0e1410e1b6589a9cd853deaa8f02f3  
 
 
 [{G}+{W}] Could you please send us the payment screenshot to our Gmail?
We will approve you within 1 hour. After that, you will be able to get unlimited cards.
Each card will have a balance of $100 to $900, which you can also use to purchase products from Amazon.
These cards are fully real cards.
Thank you
{40*"="}\x1b[1;97m""")
############------[ RANDOM NUM ]------#########
def Main():
	logo()
	print(f' {W}[{G}A{W}]{W} CARD TYPES  [{G}MASTER {W}]');print(f' {W}[{G}B{W}]{W} CARD TYPES  [{G}VISA{W}]');print(f' {W}[{G}C{W}]{W} CARD TYPES  [{G}AMEX{W}]')
	line()
...
def rmenu1():
	logo()
	if "1" in rcd:print(f"{MasterDX}");line()
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}MASTER-VISA-AMEX-CARD{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
		...
		if 'session_key' in lo:
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[MASTER-VISA-AMEX-CARD] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main ()
